...
export default function Gallery() {
  ...
  return (
    <section
      className="border-t border-surface-ring bg-background py-14 px-5 sm:py-16 sm:px-6"
      id="midias"
    >
      <div className="mx-auto max-w-6xl">
        <Reveal>
          ...
          <p className="mb-8 text-center text-foreground-muted sm:mb-10 sm:text-base">
            Toque para ampliar. Reconhece este rosto? Avise no WhatsApp.
          </p>
        </Reveal>

        <RevealStagger className="grid grid-cols-2 gap-2.5 sm:gap-3 md:grid-cols-3 lg:grid-cols-5">
          {gallery.map((item, idx) => (
            <RevealStaggerItem key={item.id}>
              <button
                type="button"
                onClick={() => setLightboxIndex(idx)}
                className="group relative aspect-square w-full overflow-hidden rounded-xl ring-1 ring-surface-ring focus:outline-none focus-visible:ring-4 focus-visible:ring-brand-red"
                aria-label={
                  item.type === "video"
